...
def main():
    ...
    for e in elasticities:
        ...
        axes[0].plot(prices,volume,label=f"Elasticity {e:g}")
        axes[1].plot(prices,profit/1000,label=f"Elasticity {e:g}")
    ...
    axes[0].set(title="Assumed demand curves",xlabel="Price (currency units)",ylabel="Units per period")
    axes[1].set(title="Contribution less fixed costs",xlabel="Price (currency units)",ylabel="Profit (thousand currency units)")
